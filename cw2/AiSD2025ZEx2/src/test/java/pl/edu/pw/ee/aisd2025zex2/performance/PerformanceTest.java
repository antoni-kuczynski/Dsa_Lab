...
public abstract class PerformanceTest {
    ...
    public static final int N_VARIANTS = 10;
    ...
    public void runPerformanceTestOnNVariantsOfSize() {
        ...
        System.out.println("\tsize\t|\tavg_time\t|\talpha");

        System.gc();
        for (int i = 0; i < N_VARIANTS; i++) {
            hash = prepareHashWithSizeAndFillIt(hashSizes[i], words);
            averageTime = measureAverageTimeOfGettingWords(words, hash);
            alpha = hash.countLoadFactor();

            System.out.println(format("\t%7d\t|\t%9d\t|\t%7f", hashSizes[i], averageTime, alpha));
        }

        assert true;
    }

    public abstract int[] getAllHashSizes();
