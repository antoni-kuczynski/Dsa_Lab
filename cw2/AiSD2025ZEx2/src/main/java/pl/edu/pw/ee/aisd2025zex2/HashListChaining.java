...
public abstract class HashListChaining<T extends Comparable<T>> implements HashTable<T> {
    ...
    public HashListChaining(int mSize) {
        validateInputHashSize(mSize);

        this.mSize = mSize;
        hashElems = new Elem[mSize];
    }
...
//        return nElem / size;
        //dodanie castowania do double, poprawa usuniecia części ułamkowej z liczby
        return (double) nElem / mSize;
    }
